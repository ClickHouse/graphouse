 ...
 * @date 16/04/2017
 */
public class MetricRowBinaryStreamCallback implements ClickHouseStreamCallback {
    ...
    @VisibleForTesting
    protected MetricRowBinaryStreamCallback(List<Metric> metrics, LocalDate localDate) {
    ...
    public MetricRowBinaryStreamCallback(List<Metric> metrics) {
    ...
    private void writeMetric(Metric metric, ClickHouseRowBinaryStream stream) throws IOException {
        ...
        stream.writeInt32((int) Integer.toUnsignedLong(metric.getTimestampSeconds()));
        stream.writeInt16(getUnsignedDaysSinceEpoch(metric.getTimestampSeconds()));
        stream.writeInt32((int) Integer.toUnsignedLong(metric.getUpdatedSeconds()));
    }
